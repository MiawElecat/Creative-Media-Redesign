        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            <a href="mailto:{{ $pesan->email }}"
                                                class="hover:text-indigo-600 dark:hover:text-indigo-400">{{ $pesan->email }}</a>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            {{ Str::limit($pesan->subjek, 30) }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 max-w-xs">
                                            <div class="truncate" title="{{ $pesan->pesan }}">
                                                {{ Str::limit($pesan->pesan, 70) }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            {{ $pesan->created_at->diffForHumans() }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            <x-danger-button type="button" x-on:click.prevent="
                                                        confirmDeleteUrl = '{{ route('admin.pesan-kontak.destroy', $pesan->id) }}';
                                                        $dispatch('open-modal', 'confirm-pk-deletion');
                                                    ">
                                                Hapus
                                            </x-danger-button>
                                        </td>
                    ...
                    <div class="mt-6">{{ $pesanKontaks->links() }}</div>
                </div>
            </div>
        </div>

        <x-modal name="confirm-pk-deletion" :show="false" focusable>
            <div class="p-6 dark:bg-gray-800">
                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">Konfirmasi Hapus Pesan</h2>
                <div class="mt-6 flex items-start space-x-3">
                    ...
                    <div class="flex-1">
                        <p class="text-sm text-gray-600 dark:text-gray-400">Apakah Anda yakin ingin menghapus pesan ini?
