    ...
    <div class="py-12" x-data="{ confirmDeleteUrl: '' }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-end mb-6">
                <a href="{{ route('admin.bidang-studi.create') }}">
                    <x-primary-button>
                        {{ __('+ Tambah Bidang Studi') }}
                    </x-primary-button>
                </a>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            ...
                                            <x-danger-button type="button" x-on:click.prevent="
                                                                    confirmDeleteUrl = '{{ route('admin.bidang-studi.destroy', $item->id) }}';
                                                                    $dispatch('open-modal', 'confirm-bs-deletion');
                                                                ">
                                                Hapus
                                            </x-danger-button>
                                        </td>
                    ...
                    <div class="mt-6">{{ $bidangStudi->links() }}</div>
                </div>
            </div>
        </div>

        <x-modal name="confirm-bs-deletion" :show="false" focusable>
            <form method="POST" x-bind:action="confirmDeleteUrl" class="p-6 dark:bg-gray-800">
                @csrf
                @method('delete')

                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">Konfirmasi Hapus Bidang Studi</h2>
                ...
                <div class="mt-6 flex items-start space-x-3">
                    ...
                    <div class="flex-1">
                        <p class="text-sm text-gray-600 dark:text-gray-400">Apakah Anda yakin ingin menghapus bidang
                            studi ini? Data yang sudah dihapus tidak dapat dikembalikan lagi.</p>
                    </div>
                </div>
