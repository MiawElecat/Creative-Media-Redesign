<x-admin-layout>
    ...
    <div class="py-12" x-data="{ confirmDeleteUrl: '' }">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            @if($artikel->is_published)
                                                <span
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200">Published</span>
                                            @else
                                                <span
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">Draft</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            ...
                                            {{-- Tombol Hapus (Hanya Pemicu Modal JS) --}}
                                            <x-danger-button type="button" x-on:click.prevent="
                                                            confirmDeleteUrl = '{{ route('admin.artikel.destroy', $artikel->id) }}';
                                                            $dispatch('open-modal', 'confirm-artikel-deletion');
                                                        ">
                                                Hapus
                                            </x-danger-button>
                                        </td>
        ...
        <x-modal name="confirm-artikel-deletion" :show="false" focusable>
            <div class="p-6 dark:bg-gray-800"> {{-- Tanpa <form> wrapper --}}
                    <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">Konfirmasi Hapus Artikel</h2>
                    <div class="mt-6 flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            <p class="text-sm text-gray-600 dark:text-gray-400">Apakah Anda yakin ingin menghapus
                                artikel ini? Tindakan ini tidak dapat dibatalkan.</p>
                        </div>
                    </div>
